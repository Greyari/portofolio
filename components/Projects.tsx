...
const projects: Project[] = [
  ...
  },
  {
    ...
    desc: "Backend HRIS cross-platform (Mobile & Web) menggunakan Laravel. Menangani manajemen karyawan, absensi, dan penggajian. Deploy ke server internal Linux/Lubuntu.",
    ...
    >
      ...
      <div className={`p-6 ${isLarge ? "md:p-8" : ""} h-full flex flex-col`}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <span className={`text-xs font-mono px-2.5 py-1 rounded-full border ${typeColors[project.type]}`}>
            {project.type}
          </span>
        ...
        <h3
          className={`font-bold text-stone-900 mb-2 leading-tight ${isLarge ? "text-2xl md:text-3xl" : "text-lg"}`}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map((cat) => (
              ...
              >
                <span style={{ fontFamily: "'Instrument Sans', sans-serif" }}>{cat}</span>
              </MagneticButton>
            ))}
          </div>
        </div>
